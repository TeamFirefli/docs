...
description: "Firefli has 5 ready-to-go cronjobs available to make changes in the background."
icon: "briefcase"
---
...
<Steps>
   ...
   * * * * * . /root/.automation_env && /usr/bin/curl -fsS -X POST 'https://your.domain.com/api/cron/update-sessions' -H "x-cron-secret: $CRON_SECRET" | /usr/bin/jq -r || true
   ...
   0 */4 * * * . /root/.automation_env && /usr/bin/curl -fsS -X POST 'https://your.domain.com/api/cron/update-users' -H "Authorization: Bearer $CRON_SECRET" | /usr/bin/jq -r || true
   ```
  ...
  <Step>
    ...
    <p>- Usernames and Displaynames will update every 4 hours.</p>
  </Step>
</Steps>
